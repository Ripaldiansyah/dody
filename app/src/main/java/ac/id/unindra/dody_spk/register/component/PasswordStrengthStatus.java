package ac.id.unindra.dody_spk.register.component;
...
public class PasswordStrengthStatus extends JPanel {
    ...
    private void checkPassword(String password) {
        this.type = password.isEmpty() ? 0 : RegisterController.checkPasswordStrength(password);
        if (type == 0) {
            label.setText("none");
            label.setVisible(false);
        } else {
            ...
            if (type == 1) {
                label.setText("Terlalu Lemah");
            } else if (type == 2) {
                label.setText("Sedang");
            } else {
                label.setText("Sangat Kuat");
            }
